...
* Better labelling of the customization for `reduce-run-MSWin-drives`.  Remove error handling from `reduce-run--run-reduce-1`, which doesn't seem helpful.  It leads to a second error "No buffer named *CSL REDUCE*" from the tidy-up code!  Consider this again later.
